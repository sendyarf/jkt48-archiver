...
export default async function PublicCatalog({ searchParams }: { searchParams: Promise<{ q?: string; member?: string; platform?: string; page?: string }> }) {
  ...
  return <div className="public-catalog">
    <section className="container catalog-hero"><div><p className="eyebrow">ARSIP REPLAY · KOMUNITAS</p><h1>Momen favorit.<br /><span>Bisa ditonton lagi.</span></h1><p className="hero-copy">Semua replay live JKT48 di satu tempat. Cari berdasarkan member, judul, atau tanggal.</p><div className="hero-actions"><a className="primary-button" href="#catalog"><PlayCircle size={18} /> Jelajahi replay</a><Link href="/members" className="text-button">Lihat member <ArrowUpRight size={16} /></Link></div></div><div className="hero-note"><span className="hero-note-mark" aria-hidden="true">▶</span><p>RINGKASAN ARSIP</p><div className="hero-stats-inline"><div><strong>{result.total}</strong><span>replay terbit</span></div><div><strong>{members.length}</strong><span>member</span></div></div><span className="hero-note-foot">Arsip komunitas, bukan situs resmi JKT48.</span></div></section>
