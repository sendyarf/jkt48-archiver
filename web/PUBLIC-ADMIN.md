...
- `TURNSTILE_SITE_KEY` / `TURNSTILE_SECRET_KEY` (opsional): Cloudflare Turnstile di `/login-r3pl4y`. Isi **keduanya** (site key publik, secret hanya server) untuk memverifikasi token sebelum password dicek; kosongkan keduanya untuk menonaktifkan. Token kosong/gagal siteverify → login ditolak 401 (fail-closed). Bila secret terisi tapi site key kosong, widget tidak tampil dan login selalu gagal.
...
## Sesi dan deployment
Sesi acak 256-bit disimpan sebagai hash di SQLite dengan expiry 8 jam; cookie HttpOnly + SameSite=Strict, logout mencabut token. Login dibatasi per-IP 20 percobaan per 15 menit melalui SQLite (tidak mempercayai header IP selain `X-Forwarded-For` pertama di belakang nginx). Opsional: set `TURNSTILE_SITE_KEY` + `TURNSTILE_SECRET_KEY` agar `POST /api/auth` memverifikasi token Turnstile ke `challenges.cloudflare.com` sebelum membandingkan password — menutup bot yang melewati rate limit tanpa CAPTCHA di origin. Tambahkan rate limit per IP pada reverse proxy tepercaya untuk mengurangi risiko lockout bersama. Pastikan DB dan file environment tidak disajikan sebagai static files; jangan cache HTML/API admin pada CDN. Backup database sebelum deployment. Tabel web_publications, web_admin_sessions, web_login_limits dibuat otomatis.
