...
- `TURNSTILE_SITE_KEY` / `TURNSTILE_SECRET_KEY` (opsional): Cloudflare Turnstile di `/login`. Isi **keduanya** (site key publik, secret hanya server) untuk memverifikasi token sebelum password dicek; kosongkan keduanya untuk menonaktifkan. Token kosong/gagal siteverify → login ditolak 401 (fail-closed). Bila secret terisi tapi site key kosong, widget tidak tampil dan login selalu gagal.
