...
CREATE TABLE live_sessions (id INTEGER PRIMARY KEY, live_id TEXT, member_username TEXT, member_name TEXT, merge_group_id INTEGER, started_at TEXT, created_at TEXT, download_ended_at TEXT, youtube_video_id TEXT, status TEXT, file_size_bytes INTEGER, platform TEXT NOT NULL DEFAULT 'idn');
...
INSERT INTO live_sessions VALUES (1, 'auto-old', 'jkt48_uji', 'Uji Member', 1, datetime('now','-100 hours'), datetime('now','-100 hours'), datetime('now','-99 hours'), 'AAAAAAAAAAA', 'done_youtube', 1, 'idn');
INSERT INTO live_sessions VALUES (2, 'auto-recent', 'jkt48_uji', 'Uji Member', 2, datetime('now','-10 hours'), datetime('now','-10 hours'), datetime('now','-9 hours'), 'BBBBBBBBBBB', 'done_youtube', 1, 'idn');
INSERT INTO live_sessions VALUES (3, 'auto-withheld', 'jkt48_uji', 'Uji Member', 3, datetime('now','-100 hours'), datetime('now','-100 hours'), datetime('now','-99 hours'), 'CCCCCCCCCCC', 'done_youtube', 1, 'idn');
INSERT INTO live_sessions VALUES (4, 'auto-fast', 'jkt48_uji', 'Uji Member', 4, datetime('now','-10 hours'), datetime('now','-10 hours'), datetime('now','-9 hours'), 'DDDDDDDDDDD', 'done_youtube', 1, 'idn');
INSERT INTO live_sessions VALUES (5, 'sr-showroom-now', 'jkt48_uji', 'Uji Member', 1, datetime('now','-4 hours'), datetime('now','-4 hours'), datetime('now','-3 hours'), 'SSSSSSSSSSS', 'done_youtube', 1, 'showroom');
...
try {
  ...
  assert.match(html, new RegExp(SHOWROOM_ID), 'Rekaman Showroom harus langsung tampil tanpa menunggu 72 jam');
  assert.doesNotMatch(html, new RegExp(RECENT_ID), 'Rekaman IDN yang belum lewat 72 jam harus tersembunyi');
  ...
  assert.equal((await fetch(s72.origin + '/watch/SSSSSSSSSSS')).status, 200, 'watch rekaman Showroom harus langsung 200');
  ...
  assert.doesNotMatch(html0, new RegExp(OLD_ID), 'Dengan auto=0, rekaman IDN lama harus tetap tersembunyi');
  assert.match(html0, new RegExp(FAST_ID), 'Publikasi manual tetap berlaku saat auto=0');
  assert.match(html0, new RegExp(SHOWROOM_ID), 'Rekaman Showroom tetap langsung tampil meski ambang IDN = 0');
  assert.equal((await fetch(s0.origin + '/watch/AAAAAAAAAAA')).status, 404, 'Dengan auto=0, watch rekaman lama harus 404');
  assert.equal((await fetch(s0.origin + '/watch/SSSSSSSSSSS')).status, 200, 'Dengan auto=0, watch Showroom tetap 200');
